...
// src/manifest.js
var ID = /^[a-z0-9][a-z0-9._/-]{2,127}$/;
var VERSION = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/;
var HASH = /^0x[0-9a-fA-F]{64}$/;
var ENTRYPOINT = /^[A-Za-z_$][A-Za-z0-9_$]*$/;
...
var MarketClient = class {
  ...
  async #write(request) {
    const hash3 = await this.wallet.writeContract(request);
    return this.public.waitForTransactionReceipt({ hash: hash3 });
  }
};
...
function renderFilters() {
  ...
  $("#filters").innerHTML = categories.map(
    (category) => '<button class="' + (state.filter === category ? "active" : "") + '" data-filter="' + category + '">' + category.toUpperCase() + "</button>"
  ).join("");
...
function renderCatalog() {
  ...
  const items = catalog.filter(
    (item) => (state.filter === "All" || item.category === state.filter) && (item.name + " " + item.description + " " + item.id).toLowerCase().includes(query)
  );
  $("#catalog").innerHTML = items.map((item) => {
    const installed = state.installed.includes(item.id);
    return '<article class="card ' + (state.selected?.id === item.id ? "selected" : "") + '" data-id="' + item.id + '"><div class="card-top"><span class="card-icon">' + item.icon + '</span><span class="card-category">' + item.category.toUpperCase() + "</span></div><h2>" + escapeHtml(item.name) + "</h2><p>" + escapeHtml(item.description) + '</p><div class="card-foot"><div class="price"><strong>' + item.price + "</strong><small>" + item.term + '</small></div><button data-buy="' + item.id + '">' + (installed ? "INSTALLED" : item.wei === 0n ? "INSTALL" : "ACQUIRE") + "</button></div></article>";
...
function renderInspector() {
  ...
  const installed = state.installed.includes(item.id);
  $("#inspector").innerHTML = '<div class="inspect-label">COMPONENT / ' + escapeHtml(item.category.toUpperCase()) + "</div><h2>" + escapeHtml(item.name) + '</h2><div class="publisher">PUBLISHED BY ' + item.developer + '</div><p class="description">' + escapeHtml(item.description) + '</p><div class="inspect-section"><h3>REQUESTED CAPABILITIES</h3>' + item.permissions.map((permission) => '<div class="permission"><span>' + permission + '</span><b class="safe">DECLARED</b></div>').join("") + '</div><div class="inspect-section"><h3>LICENSE</h3><div class="license-box"><div><span>PRICE</span><b>' + item.price + "</b></div><div><span>DURATION</span><b>" + item.term + "</b></div><div><span>STATUS</span><b>" + (installed ? "INSTALLED" : "AVAILABLE") + '</b></div></div></div><div class="inspect-actions"><button class="action primary" id="inspectAcquire">' + (installed ? "LAUNCH COMPONENT" : item.wei === 0n ? "INSTALL LOCALLY" : "ACQUIRE LICENSE") + '</button><button class="action" id="inspectSource">VIEW COMPONENT ID</button></div>';
...
function renderInstalled() {
  ...
  $("#installedList").innerHTML = items.map(
    (item) => '<article class="installed-item"><span class="card-icon">' + item.icon + "</span><div><strong>" + escapeHtml(item.name) + "</strong><small>" + escapeHtml(item.id) + ' \xB7 VERIFIED</small></div><div><button class="action primary" data-launch="' + item.id + '">LAUNCH</button> <button class="action" data-remove="' + item.id + '">REMOVE</button></div></article>'
...
function renderActivity() {
  $("#activityList").innerHTML = state.activity.map(
    (entry) => '<article class="activity-item"><time>' + new Date(entry.time).toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit"
    }) + "</time><div><b>" + escapeHtml(entry.action) + "</b><small>" + escapeHtml(entry.item) + "</small></div><em>" + escapeHtml(entry.status) + "</em></article>"
...
function openView(name) {
  ...
  document.querySelectorAll("#navigation button").forEach(
    (button) => button.classList.toggle("active", button.dataset.view === name)
  );
...
function tick() {
  $("#clock").textContent = (/* @__PURE__ */ new Date()).toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit"
  });
}
if (window.ethereum?.on) {
  window.ethereum.on("accountsChanged", (accounts) => {
    ...
    $("#identityText").textContent = state.account ? state.account.slice(0, 6) + "\u2026" + state.account.slice(-4) : "LOCAL USER";
